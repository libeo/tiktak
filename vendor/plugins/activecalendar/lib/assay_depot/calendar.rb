    ...
    module DepotDateHelper
      def depot_date_select(options = {})
        ...
        if options[:date] == nil
          ...
          date = options[:date].strftime(options[:ifFormat])
        end

        html = ""
		########################################################################
		# Commented this out because it raised an error, and it doesn't seem to be useful anymore
		########################################################################
		...
		calendar_options[:date] = "new Date(%s)" % calendar_options[:date].to_time.to_i if calendar_options[:date]
        calendar_options.delete_if { | key, value |
          [ :name, :class, :id, :object_name, :method ].include? key or value == nil
        }

        html << %(<script type="text/javascript">\n)
        html << %(    Calendar.setup\({\n)
        calendar_options.each { | key, value |
          if(key.to_s.eql?('date') || key.to_s.eql?('dateStatusFunc') || !value.instance_of?(String))
            html << %(        #{key} : #{value},\n )
          else
            html << %(        #{key} : "#{value}",\n )
          end
...
#        html << %(        button         :    "#{options[:id]}_trigger",  // trigger for the calendar, button ID\n)
      ...
      def depot_datetime_select(options = {})
        notStrings = [:date, :onClose, :onSelect]
        ...
        if options[:date] == nil
          ...
          datetime = options[:date].strftime(options[:ifFormat])
        end
        ...
        calendar_options["inputField"] ||= "#{options[:id]}"
        calendar_options["button"] ||= "#{options[:id]}_trigger"
		calendar_options[:date] = "new Date(%s)" % calendar_options[:date].to_i
        calendar_options.delete_if { | key, value |
        ...
        html << %(    Calendar.setup\({\n)
        calendar_options.each { | key, value |
          if(key.to_s.eql?('dateStatusFunc') || !value.instance_of?(String) || notStrings.include?(key) )
            html << %(        #{key} : #{value},\n )
          else
            html << %(        #{key} : "#{value}",\n )
          end
    ...
    module DateHelper
      def date_select(object_name, method, options = {}, other={})
		...
		#InstanceTag.new no longer uses 5 arguments, only 4
        InstanceTag.new(object_name, method, self, options.delete(:object)).to_date_select_tag(options)
      end

      def datetime_select(object_name, method, options = {}, other={})
        name = options[:name].nil? ? "#{object_name}[#{method}]" : options[:name]
        id = options[:id].nil? ? "#{object_name}_#{method}" : options[:id]
        options = options.merge( { :name => name, :id => id, :method => method, :object_name => object_name })
		#InstanceTag.new no longer uses 5 arguments, only 4
